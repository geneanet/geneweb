...
let _conf_line_piqi_type = Piqirun_ext.find_piqi_type "api_saisie_read/conf-line"
...
let parse_conf_line ?opts x (format :Piqirun_ext.input_format) =
  let x_pb = Piqirun_ext.convert _conf_line_piqi_type format `pb x ?opts in
  let buf = Piqirun.init_from_string x_pb in
  Api_saisie_read_piqi.parse_conf_line buf
...
let gen_conf_line ?opts x (format :Piqirun_ext.output_format) =
  let buf = Api_saisie_read_piqi.gen_conf_line x in
  let x_pb = Piqirun.to_string buf in
  Piqirun_ext.convert _conf_line_piqi_type `pb format x_pb ?opts
...
let print_conf_line ?opts x =
  Pervasives.print_endline (gen_conf_line x `piq ?opts)
let prerr_conf_line ?opts x =
  Pervasives.prerr_endline (gen_conf_line x `piq ?opts)
